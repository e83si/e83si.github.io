...
const bundleLinkMsg = 'The download will be a .zip file\n'
      + ' Recommended to use 7-zip or WinRAR to open it.\n'
      + ' Windows built in zip tool usually gives an error.\n'
      + ' Download may take a long time to start (60 seconds)';

function bundleLinkClicked(ev) {
    if (!confirm(bundleLinkMsg)) {
	ev.preventDefault();
    }
}

function main() {
    [].forEach.call(document.querySelectorAll('.bundle-link'), elt => {
	elt.addEventListener('click', bundleLinkClicked);
    });
